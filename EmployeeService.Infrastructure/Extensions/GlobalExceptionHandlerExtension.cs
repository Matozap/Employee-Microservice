        ...
        private static Action<IApplicationBuilder> HandleApiException(ILoggerFactory loggerFactory, IWebHostEnvironment env)
        {
            return appBuilder =>
            {
                appBuilder.Run(async context =>
                {
                    ...
                    {
                        var logger = loggerFactory.CreateLogger("NID Global Exception");
                        ...
                        string result;
                        if (!env.IsDevelopment() && !(exceptionHandlerFeature.Error is ValidationException))
                        {
                            result = JsonSerializer.Serialize(new
                            {
                                Error =
                                    $"An unexpected error happened. - Detail: {exceptionHandlerFeature.Error.Message}"
                            });
                        }
                        else
                        {
